...
NockApp is a framework for building applications whose business logic is written in Nock and runs inside a Rust host runtime.  Nockchain is one such application:  a Proof-of-Work blockchain in which the work being done is the generation of zero-knowledge proofs over [`NockVM`](https://docs.nockchain.org/reference-runtime/nockvm) execution traces.

This page describes how those pieces fit together and which properties of Nock they rely on.  It is not a tutorial for either system; for hands-on material, see [docs.nockchain.org](https://docs.nockchain.org).

## The Two-Component Pattern

A NockApp is split into a **kernel** and a **runtime**:

* The **kernel** is a Nock noun—usually compiled from Hoon—that holds the application's state and exposes a small standard interface.  It is the part that is, in principle, formally provable.
...
The kernel presents three arms that the runtime invokes:
...
This is the same kernel shape used by Arvo, covered on the [Kernels](kernel.md) page; NockApp standardizes it and ships a runtime that knows how to drive it.  The runtime persists state to a **Persistent Memory Arena (PMA)**—a file-backed arena that demand-pages the kernel's noun so the working set fits in memory regardless of total state size.

NockApp instances communicate via gRPC, which the Nockchain docs describe as the framework's interop layer.

## Nock as a Proof Target

Nockchain's central technical bet is that Nock makes a usable target for STARK-based zero-knowledge proofs.  Per the [Nockchain documentation](https://docs.nockchain.org/architecture/why-nockchain):
...
The efficiency claim is theirs, and is presented relative to RISC-V-family ZKVMs; the documentation does not break the comparison down further on the public docs site.

A few Nock properties shape the proof system:

* **Small instruction set.**  Twelve opcodes mean a small set of circuit constraints to instantiate.
...
The earliest published treatment of Nock-as-zk-target is referenced from the [Specification](../specification/index.md):  Kattis, Klatt, Quirk & Allen (2025), *A Framework for Compiling Custom Languages as Efficiently Verifiable Virtual Machines* (IACR ePrint 2025/1110).
...
Whether this trade-off (proving cost shifted off-chain, verification cost held roughly constant) is a net win in practice depends on workload and on how cheap proving turns out to be.
...
This is a design choice with active tradeoffs being discussed in the broader ZK-rollup space, and Nockchain is one of several systems exploring it; the framing on docs.nockchain.org is its own.  The chain's emission curve is also unusual:  `$NOCK` is hard-capped at 2³² units (approximately 4.29 billion) and minted only as PoUW rewards.

## Where Nock-the-Language Sits

If you've worked through this site's [Specification](../specification/index.md), the parts of Nock that matter most for Nockchain are:

* **[Opcode 11 (Hint)](../specification/opcode-11.ipynb)** — how the kernel asks the runtime for side effects.  Jets are signalled via `%fast` hints; logging via `%slog`; profiling via `%bout`.
* **[Opcode 12 (Scry)](virtualization.md)** — added by the virtualized layer.  `+peek` resolves through it in NockApp.
* **[Cores, gates, and arms](../glossary.md)** — the kernel is structurally a core with `+load`/`+peek`/`+poke` as named arms.
* **[Solid-state semantics](../specification/deep-nock.md)** — why a Nock noun is a complete portable state record.

NockApp is, in effect, the [`++mink`/`++mock` interpreter pattern](virtualization.md) packaged with a Rust I/O layer and a set of conventions about how state and upgrades work.
